...
* @revdate $Date:   20 Mar 2007 10:44:24  $
*/
...
if(typeof(s_account) == "undefined"){
	var s_account="homehamptonbaydev"
}
...
s.visitorNamespace="homedepot"
s.trackingServer="wasc.homedepot.com"
s.trackingServerSecure="swasc.homedepot.com"
s.dc=112
...
s.linkInternalFilters="javascript:,homedepot.com,homedepot.shoplocal.com,gardenclub.com,homedepotgardenclub.com,homedepot.bighammersoftware.com"
...
/* Plugin Config (prior to doPlugins) */
...
function s_getObjectID(o) {
	 ...
	 *       be created, return ''.
	 */
	var h=o.href, str, i, j;
	h=s.repl(h,'"','');
	i=h.indexOf('//');
	if(i!=-1) {
		str=h.substring(i+2,h.length);
		i=str.indexOf('/')
		if(i!=-1) {
	...
	if(!str) str=h;
	i=str.indexOf('BV_SessionID');
	if(i!=-1) {
		j=str.indexOf('@@@@&');	 	
		str=str.substring(0,i)+str.substring(j+5,str.length);
	}
	i=str.indexOf('&BV_EngineID');
	if(i!=-1) {
		j=str.indexOf('&MID');	 	
	...
	if(i!=-1) {
		j=str.length;
		if(j>i+8){
			str=str.substring(0,i)+str.substring(i+8,j);
		}
		else{
...
function s_doPlugins(s) {
	/* Add calls to plugins here */
 	var temp=s.getQueryParam('cm_mmc','','f');
 	if(temp) {
		s.campaign=temp;
		s.eVar36=s.getDaysSinceVal(s.campaign,'s_v36',30,0);
		s.eVar37=s.getDaysSinceVal(s.campaign,'s_v37',30,1);
	} else {
		temp=s.getQueryParam('cm_ven,cm_cat,cm_pla,cm_ite','-_-','f');
  		if (temp) {
			s.campaign=temp;
			s.eVar36=s.getDaysSinceVal(s.campaign,'s_v36',30,0);
			s.eVar37=s.getDaysSinceVal(s.campaign,'s_v37',30,1);
 	...
 	/* Don't allow same campaign on subsequent pages */
 	if (s.exec<1) {
 		var k=s.c_r("s_cmpnm")
 		s.c_w("s_cmpnm",s.campaign)
		if(s.campaign==k)
			s.campaign=""
		s.exec++;
	}
	...
	if(s.getQueryParam('s_kwcid')) {
		s.pageURL=s.swapQueryParam('s_kwcid');
		}

	s.eVar1 = s.campaign;
	s.eVar43 = s.campaign;

	/* if orig_ref param exists populate s.referrer */
	var oref=s.getQueryParam('orig_ref');
	if(oref) s.referrer=oref;

	if(!s.eVar2){
		s.eVar2=s.getQueryParam('cm_sp','','f');
		s.eVar2=s.getValOnce(s.eVar2,"ev2",0);
		}
	if(!s.eVar38)
		s.eVar38=s.getQueryParam('adref','','f');
 	s.setupDynamicObjectIDs();
 	s.events=s.getCartOpen("s_scOpen");
 	s.events=s.apl(s.events,'event10',',','1');  // Page View Event
 		temp=s.getValOnce(s.prop4,'s_p4',0)
  	if(temp) {
   		s.events=s.apl(s.events,'event15',',','1');  // Search Event
  	}
	s.prop18="Cookied Visits";
 	s.prop18=s.getValOnce(s.prop18,"s_p18",.020833);
}

s.doPlugins=s_doPlugins

function s_splitOrder() {
	if (s.disableTracking) return;
	var linkName="Split Order";
	var imgSrc,tm=new Date(),yr=tm.getYear();
	yr=yr<1900?yr+1900:yr;
	t=tm.getDate()+'/'+tm.getMonth()+'/'+yr+' '+tm.getHours()+':'+tm.getMinutes()+':'+tm.getSeconds()+' '+tm.getDay()+' '+tm.getTimezoneOffset();
	imgSrc="http"+(s.ssl?"s":"")+"://"+(s.ssl?'swasc':'wasc')+'.homedepot.com/b/ss/'+s.un+'/1/H.6-pdv-2/';
	imgSrc+="s"+tm.getTime()+"-"+Math.round(Math.random()*1000)
	imgSrc+="?[AQB]"
	imgSrc+="&t="+s.ape(t);
	imgSrc+="&ns="+s.visitorNamespace
	imgSrc+="&g="+s.ape(window.location)
	imgSrc+="&pe=lnk_o"+"&pev2="+s.ape(linkName)
	if(s.currencyCode){imgSrc+="&cc="+s.ape(s.currencyCode)}
	if(s.state){imgSrc+="&state="+s.ape(s.state)}
	if(s.zip){imgSrc+="&zip="+s.ape(s.zip)}
	if(s.purchaseID){imgSrc+="&purchaseID="+s.ape(s.purchaseID)}
	if(s.events){imgSrc+="&events="+s.ape(s.events)}
	if(s.products){imgSrc+="&products="+s.ape(s.products)}
	imgSrc+="&[AQE]";
	imgTag='<img src="'+imgSrc+'" width="1" height="1" alt="">';
	document.write(imgTag);
}
s.splitOrder=s_splitOrder

/************************** PLUGINS SECTION *************************/
/* You may insert any plugins you wish to use here.                 */
/*
 * Plugin: getDaysSinceVal v1.0
 */
s.getDaysSinceVal=new Function("v","c","e","n",""
+"var s=this,r=-1,p,i,pt,pv,t=new Date,D=86400000;"
+"p=s.c_r(c);"
+"if(p){"
	+"i=p.indexOf('#');"
	+"if(i>0){"
		+"pt=p.substring(0,i);"
		+"pv=p.substring(i+1,p.length);"
		+"r=Math.floor((t.getTime()-pt)/D);"
	+"}"
+"}"
+"if(v){"
	+"if(n==0){r=0;pt=t.getTime()}"
	+"else if(v!=pv){r=0;pt=t.getTime()}"
	+"p=''+pt+'#'+v;"
	+"t.setTime(t.getTime()+D*e);"
	+"if(!s.c_w(c,p,t)){r='na'}"
+"}"
+"if(r===-1){r=''}"
+"if(r===0){r='same day'}"
...
+"var s=s_c_il["+s._in+"],b=s.eh(s.wd,'onload'),o='onclick',x='',l,u,"
...
/*
 * Plugin: swapQueryParam 1.0 - swap parameters in query string 
 */

s.swapQueryParam=new Function("p","u",""
+"var s=this,v='',i,t;"
+"var q=s.getQueryParam(p);"
+"u=u?u:(s.pageURL?s.pageURL:''+s.wd.location);"
+"u=u=='f'?''+s.gtfs().location:u;"
+"if(q){var new_q=q;if(q.indexOf('%20')>-1) new_q=unescape(q);"
+"i=new_q.indexOf('|');if(i>-1){x=new_q.substring(0,i);"
+"new_q=escape(x)+new_q.substring(i);}var x=u.indexOf('=',u.indexOf(p));"
+"var a=u.substring(0,x+1);var x=u.indexOf('&',u.indexOf(p));"
+"var b=x>-1?u.substring(x):'';u=a+new_q+b;u = s.rep(u,'&'+p+'='+new_q,'');"
+"u = s.rep(u,p,p+'old');u = s.rep(u,'?','?'+p+'='+new_q+'&');"
+"return u}"
);
